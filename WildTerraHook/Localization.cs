...
MISC_BORDERLESS=Force Borderless Window
ACTION_TITLE=Auto Actions (World)
ACTION_ENABLE=Enable Auto Actions
ACTION_RANGE=Action Range
ACTION_DELAY=Action Delay (s)
ACTION_SELECT=Select Action
SKILLUNLOCK_TITLE=Skill Bar Unlocker
SKILLUNLOCK_ENABLE=Force 20 slot bar
SKILLUNLOCK_INFO=Requires re-equip/re-login to refresh UI.
FEEDER_TITLE=Auto Pet Feeder
FEEDER_ENABLE=Enable Auto Feeding
ANTIAFK_ENABLE=Enable Anti-AFK Movement
...
MISC_BORDERLESS=Wymuś Okno Bez Ramek (Borderless)
FEEDER_TITLE=Auto Karmienie Zwierząt
FEEDER_ENABLE=Włącz Auto Karmienie
ANTIAFK_ENABLE=Włącz Anti-AFK Ruch
ACTION_TITLE=Auto Akcje (Świat)
ACTION_ENABLE=Włącz Auto Akcje
ACTION_RANGE=Zasięg Akcji
ACTION_DELAY=Opóźnienie Akcji (s)
ACTION_SELECT=Wybierz Akcje
SKILLUNLOCK_TITLE=Odblokowanie Paska Umiejętności
SKILLUNLOCK_ENABLE=Wymuś pasek na 20 slotów
SKILLUNLOCK_INFO=Wymaga ponownego założenia/ponownego logowania, aby odświeżyć UI.
